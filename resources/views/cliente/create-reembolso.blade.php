...
<div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md">
    ...
    <form action="{{ route('reembolsos.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="pedido" value="Pedido #{{ $pedido->id }}" class="block w-full mt-2 border-gray-300 rounded-md" disabled>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="monto" step="0.01" class="block w-full mt-2 border-gray-300 rounded-md" value="{{ old('monto') }}" required>
            @error('monto')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
