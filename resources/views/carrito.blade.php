        ...
        @if (count($carrito) > 0)
            <div class="bg-white rounded-lg shadow-lg p-6">
                            ...
                            <tr data-id="{{ $item['id'] }}">
                                ...
                                <td class="border-b p-4">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <input type="text" value="{{ $item['cantidad'] }}" min="1" max="20"
                                ...
                                <td class="border-b p-4 subtotal">S/
                                ...
                                <td class="border-b p-4">
                                    <form action="{{ route('carrito.eliminar', $item['id']) }}" method="POST">
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded">Eliminar</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
                ...
                <div class="text-right mt-4">
                    <h2 class="text-lg font-semibold">Total: S/ <span id="total">{{ number_format($total, 2) }}</span>
                    </h2>
                    <a href="{{ route('resumen.pedido') }}" class="bg-blue-500 text-white px-4 py-2 mt-4 rounded">Realizar
                        pago</a>
                </div>
            </div>
        @else
